...
/**
	 ...
	 * @return	void
	 */
	public function build_cache($cache, $data = array(), $return = FALSE)
	{
		// Set whatever values are given. These will be available to all view files
		is_array($data) OR $data = (array) $data;
	...
	/**
	 * load_view
	 * Load a partial view from theme partial folder
	 *
	 * @access	public
	 * @param	 string	$view
	 * @return	array
	 * @author Marius Baltag
	 */
	public function load_view($view, $data = array(), $return = FALSE)
	{
		...
		if ( ! $return)
		{
			echo self::_find_view($view, $data, $return);
		}
		else
		{
			return self::_find_view($view, $data, $return);
		}
	}
